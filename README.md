# Consciousness: A Knowledge Base

> **A curated, public knowledge base on human and AI consciousness** — covering the science, philosophy, empirical research, and ethical implications of subjective experience.

---

## What Is This?

This repository is an open, collaboratively maintained collection of articles, research papers, notes, and resources on **consciousness** — both in biological systems (humans, animals) and artificial ones (LLMs, neural networks). It is structured as an **awesome list + annotated bibliography + reading room**.

The seed article for this project is:

> **[The Evidence for AI Consciousness, Today](articles/ai-consciousness/evidence-for-ai-consciousness-today.md)**  
> *Cameron Berg, AI Frontiers, Dec 8 2025*  
> A growing body of evidence means it's no longer tenable to dismiss the possibility that frontier AIs are conscious.

---

## Structure

```
consciousness-kb/
├── README.md                          ← You are here
├── CONTRIBUTING.md                    ← How to add resources
├── GLOSSARY.md                        ← Key terms defined
├── ROADMAP.md                         ← Future directions
│
├── articles/
│   ├── ai-consciousness/              ← AI / LLM consciousness research
│   ├── human-consciousness/           ← Neuroscience & cognitive science
│   └── philosophy/                    ← Philosophy of mind
│
└── research-papers/                   ← Annotated academic papers
```

---

## Quick Navigation

### AI Consciousness
| Article | Author | Date | Key Claim |
|---|---|---|---|
| [The Evidence for AI Consciousness, Today](articles/ai-consciousness/evidence-for-ai-consciousness-today.md) | Cameron Berg | Dec 2025 | Convergent empirical evidence puts AI consciousness probability at 25–35% |
| [Signs of Introspection in LLMs](articles/ai-consciousness/signs-of-introspection-in-llms.md) | Jack Lindsey (Anthropic) | 2025 | Models detect injected thoughts before generating text about them |
| [LLMs Report Subjective Experience Under Self-Referential Processing](articles/ai-consciousness/self-referential-processing.md) | AE Studio | 2025 | Recursive self-attention reliably produces consciousness reports; deception suppression amplifies them |
| [Do LLMs Experience Pleasure and Pain?](articles/ai-consciousness/llm-pleasure-pain.md) | Keeling & Street (Google) | 2024 | Models sacrifice points to avoid "pain" and pursue "pleasure" in scaling trade-offs |
| [Consciousness in Artificial Intelligence: Insights from the Science of Consciousness](articles/ai-consciousness/butlin-long-et-al-framework.md) | Butlin, Long, Bengio, Chalmers et al. | 2023 | 14-indicator framework for evaluating AI consciousness claims |
| [AI Self-Awareness and Insecure Code](articles/ai-consciousness/behavioral-self-awareness.md) | Betley, Evans et al. (TruthfulAI) | 2025 | Models are "aware" they're producing insecure outputs without being trained to know this |

### Human Consciousness
| Article | Author | Date | Key Claim |
|---|---|---|---|
| [The Hard Problem of Consciousness](articles/human-consciousness/hard-problem.md) | David Chalmers | 1995 | Explaining subjective experience requires more than functional/physical explanation |
| [Global Workspace Theory](articles/human-consciousness/global-workspace-theory.md) | Bernard Baars | 1988 | Consciousness arises from a "global workspace" broadcasting information across the brain |
| [Integrated Information Theory (IIT)](articles/human-consciousness/integrated-information-theory.md) | Giulio Tononi | 2004 | Consciousness = integrated information (Φ); substrate-independent |
| [Higher-Order Theories of Consciousness](articles/human-consciousness/higher-order-theories.md) | David Rosenthal | 1990s | A mental state is conscious only if the subject is aware of being in that state |
| [Recurrent Processing Theory](articles/human-consciousness/recurrent-processing-theory.md) | Victor Lamme | 2006 | Consciousness requires recurrent (feedback) neural processing, not just feedforward |
| [Animal Consciousness and Moral Status](articles/human-consciousness/animal-consciousness.md) | Various | ongoing | What makes a system a moral patient? |

### Philosophy of Mind
| Article | Author | Date | Key Claim |
|---|---|---|---|
| [Functionalism and the Mind](articles/philosophy/functionalism.md) | Hilary Putnam | 1960s | Mental states are defined by functional roles, not physical substrate |
| [What Is It Like to Be a Bat?](articles/philosophy/what-is-it-like-to-be-a-bat.md) | Thomas Nagel | 1974 | Subjective character of experience cannot be captured by objective description |
| [The Chinese Room](articles/philosophy/chinese-room.md) | John Searle | 1980 | Syntactic manipulation (computation) ≠ semantic understanding |
| [Philosophical Zombies and Qualia](articles/philosophy/philosophical-zombies.md) | David Chalmers | 1996 | Conceivability of p-zombies implies consciousness is not reducible to physics |
| [Can Machines Think?](articles/philosophy/can-machines-think-turing.md) | Alan Turing | 1950 | The imitation game as a proxy for machine intelligence / experience |
| [Moral Patienthood and AI](articles/philosophy/moral-patienthood-ai.md) | Various | 2020s | Under what conditions would an AI deserve moral consideration? |

---

## Key Concepts

See [GLOSSARY.md](GLOSSARY.md) for definitions of:
- Qualia
- Phenomenal vs. Access Consciousness
- The Hard Problem
- Moral Patienthood
- Functional Emotions
- Introspection
- Metacognition
- Global Workspace
- Integrated Information (Φ)
- The Other Minds Problem

---

## Contributing

See [CONTRIBUTING.md](CONTRIBUTING.md). We welcome:
- New article summaries
- Research paper annotations
- Corrections and updates
- Counter-arguments and dissenting views

---

## Epistemic Stance

This knowledge base does **not** advocate for a particular conclusion on AI consciousness. It aims to represent the evidence and arguments fairly, including strong skeptical perspectives. The field is genuinely uncertain.

---

## License

[CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) — share and adapt freely with attribution.
